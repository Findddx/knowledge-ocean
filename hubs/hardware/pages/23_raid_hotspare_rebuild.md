...
### RAID rebuild 风险不能只算容量

最乐观估算可以先写成：

```text
最乐观重建时间 ~= 需要重建的数据量 / 可持续重建吞吐
实际风险窗口 = 最乐观重建时间 + 限速 + 业务写入干扰 + 重试/坏块 + 人工响应
```

| 放大因素 | 为什么会放大风险窗口 | 设计动作 |
| --- | --- | --- |
| 单盘容量变大 | 必须读写更多数据，重建时间增长 | 重新评估 RAID 5/6/10，而不是沿用旧模板 |
| 业务高峰写入 | rebuild 与生产 I/O 争盘、争缓存、争链路 | 设置 rebuild rate 和维护窗口 |
| 隐藏介质错误 | 重建时才第一次读到坏块 | 固定 Patrol Read / Consistency Check |
| 同批次老化 | 多块盘在同一时间段进入高风险期 | 分批采购、分散批次、记录运行小时 |
| 高温与背板问题 | 重建期间盘和控制器压力上升 | 检查风道、温度告警和 enclosure 事件 |

重建前后最小检查清单：

- 换盘前确认故障盘槽位、序列号、阵列名和 foreign config 状态。
- 换盘后确认 hot spare 接管范围、rebuild 方向和预计完成时间。
- 重建期间监控第二块盘的 predictive failure、介质错误、timeout、温度和链路 reset。
- 重建完成后执行 consistency check，并确认备份链仍有可用恢复点。
