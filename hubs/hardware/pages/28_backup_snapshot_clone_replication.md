...
| 问题 | 如果答案是“否”，风险是什么 |
| --- | --- |
| 副本是否离开了原存储故障域？ | 阵列、pool 或站点故障会一起带走恢复点 |
| 副本是否离开了原权限域？ | 生产管理员或被盗凭据可能删除所有副本 |
| 副本是否有多个历史点？ | 误删、勒索和逻辑损坏会传播到唯一副本 |
| 副本是否有独立目录/索引？ | 数据块存在但恢复软件找不到对象 |
| 副本是否定期恢复验证？ | “任务成功”不能证明业务可用 |
| 副本是否有 retention/lock 保护？ | 恢复点可能在事故前被清理或缩短保留 |
